...
`<em>` — `</em>`
: виділяє текст курсивом. Також існує застарілий тег `<i>`, який виділяє текст курсивом, але не має семантичного значення, тому його не рекомендується використовувати.
...
`<div>` — `</div>`
: використовується для логічного виділення блоку HTML-документа. Використовується як зручний контейнер для об'єктів сторінки, яким легко динамічно маніпулювати — переміщати, вмикати / вимикати, створювати шари, регулювати відступи та інше.

Інший варіант — `<span>` — так само використовується для групування тексту, однак на відміну від `<div>` не створює нового блоку, а дозволяє виділити частину тексту в межах блоку.

---

# Атрибути тегів

Теги також можуть мати атрибути. В загальному випадку це пари ім’я-значення, розділені знаком `=`, які записуються всередині відкриваючого тегу. 

Наприклад, щоб створити посилання на іншу сторінку необхідно додати тег `<a>`, а URL, на який відбуватиметься перехід, вказати за допомогою атрибуту `href`: 
...
```html
<img src="images/logo.jpg" width="50px" height="50px" alt="Nice image" align="left"/>Текст справа
```
Виглядатиме це так:

<img src="https://placehold.co/90x90" width="90px" height="90px" alt="Nice image" align="left"/> Текст справа<br/><br/>
...
Щоб задати іконку її можна теж підключити тегом `<link>`:
